...
//this must be cleared?? otherwise if you decrease the size and it hasnt been shut to stop persistence then its still got more in it = nullpointer because its used as the loop i<?
        ...
        for (int i = 0; i < ActProfiles.profileDatastore.profiles.size(); i++) {
            int profileID= ActProfiles.profileDatastore.profiles.get(i).databaseNum;
...
/////////////////
    ...
    public void saveToParse(){
        ...
        for (int ii = 0; ii < JSONgoals.size(); ii++) {
            ...
            String profileName = ActProfiles.profileDatastore.profiles.get(ii).name;
            int profileID = ActProfiles.profileDatastore.profiles.get(ii).databaseNum;
            ...
            goal.put("Count", ActProfiles.profileDatastore.count);
                ...
                if (e == null) {
                    ...
                    if (goalData.size() < 1) {
                        ...
                        for(int i=0; i< ActProfiles.profileDatastore.profiles.size(); i++){
                            ...
                            ActProfiles.profileDatastore.removeProfile(ActProfiles.profileDatastore.getProfile(i));

                        }//fix this to reference an instance of profileDatastore? rather than contacting MainActivity so much.
                        ActProfiles.profileDatastore.profiles.clear();
                        for (ParseObject goalRow : goalData) {
                            ...
                            try {
                                ...
                                //int refreshDayOfYear = ActGoals.goalStore.dayofyear + ActGoals.goalStore.daysToRefresh();
